...
module.exports={
    ...
    run: async(bot,message,args)=>{
      ...
      if(logs === channel.id) return message.channel.send(errorEmbed.setDescription("That is already your logs channel!"))
      
      if(args[1]) return message.channel.send(errorEmbed.setDescription("Invalid formatting!"))
      database.set(`modlogs_${message.guild.id}`, channel.id)
      
      const successEmbed = new MessageEmbed()
      .setTitle("Success!")
      .setDescription(`I have set the mod logs channel to ${channel}`)
      .setColor("GREEN")
      .setTimestamp()
